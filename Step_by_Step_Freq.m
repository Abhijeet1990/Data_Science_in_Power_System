% read the first 5000 columns of original frequency data (in Hz)
Freq_Data_Hz = csvread('NASPI-2014-Workshop-Oscillation-Case1.csv',4,1,[4,1,5000,5]);

% plot original frequency data (in Hz)
figure(1);
plot(Freq_Data_Hz,'DisplayName','Freq_Data_Hz');
grid on;

% get per unit values of frequency data (in pu)
Freq_Data_pu = Freq_Data_Hz / 60;

% plot per unit frequency data (in pu)
figure(2);
plot(Freq_Data_pu,'DisplayName','Freq_Data_pu');
grid on;

%---------- start of running PCA through SVD ----------%
Freq_Data_No_Mean_pu = detrend(Freq_Data_pu, 'constant'); % remove column means from per-unit frequency values

X = Freq_Data_No_Mean_pu';  % get X matrix
...
Freq_Data_pu_Proj = Y';   % correspond to 'score' output of pca()

% plot projection of original measurements onto new basis P
figure(3);
plot(Freq_Data_pu_Proj,'DisplayName','Freq_Data_pu_Proj');
grid on;
...
figure(8);
a = 20;
c = linspace(1,10,length(Freq_Data_pu_Proj(:,1)));
scatter(Freq_Data_pu_Proj(:,1),Freq_Data_pu_Proj(:,2),a,c);
grid on;
...
a = 20;
c = linspace(1,10,length(Freq_Data_pu_Proj(:,1)));
scatter3(Freq_Data_pu_Proj(:,1),Freq_Data_pu_Proj(:,2),Freq_Data_pu_Proj(:,3),a,c);

[coeff,score,latent,tsquared,explained,mu] = pca(Freq_Data_pu);
